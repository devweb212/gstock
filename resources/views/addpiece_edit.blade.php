...
<input type="text" class="form-control" id="validationCustom01" placeholder="Nom d'article" value="{{$edit->name_piece}}"  name="name_piece" required>
    ...
    <input type="text" class="form-control" id="validationCustomUsername" placeholder="Référence" aria-describedby="inputGroupPrepend" name="refer"  value="{{$edit->ref}}"  required>
        ...
        <input type="number" class="form-control" id="validationCustomUsername" placeholder="Quantité Initiale" aria-describedby="inputGroupPrepend" name="inventaire" value="{{$edit->inventaire}}"  required>
...
<div class="col-md-4 mb-3">
                  <label for="validationCustom05">Motif pour la Modification  </label>
                  <textarea name="obser" id="exampleText" class="form-control" required></textarea>
...
<td>
									...
									@foreach ($listeInvpieces as $listepiec2)			
												
											 @if ($listepiec2->id==$listepiec->equiv )
												{{$listepiec2->name_piece}}
											  @endif	 
										@endforeach		
...
<td><?php echo '<a href="data:image/png;base64,' . DNS1D::getBarcodePNG(request()->id, "C39+",3,70) . '"  download="CodeBar_'.$listepiec->name_piece.'_'.$listepiec->ref.'"><i class="fa fa-download icon-gradient bg-malibu-beach"> </i> </a>';?>
	<a href="/addpieces/{{$listepiec2->id}}"> 
